...
\section{Gaussian Unitary Ensemble} % (fold)
...
constant $C_N$ right now, but later in \Cref{} we will see that it is equal to 
$\dfrac{(2\pi)^{-N/2}}{0! 1!\cdots (N-1)!}$.
\begin{proof}
  ...
  (This implies that $f$ depends only on the eigenvalues $\lambda_1,\cdots, \lambda_N$ of $A$, and 
  moreover, in a symmetric way.) We will show that
  \begin{equation*}
    \int_{\mathcal H_N}f(A) dA=\mathrm{const}_N\int_{\mathbb{R}^N} f(\lambda_1,\cdots, \lambda_N) 
  ...
  Consider $\mathcal H_N$ as a space with an action of $U(N)$: $A\mapsto UAU^*$. A typical $A\in \mathcal H_N$ can be written as $U \Lambda U^*$, where $\Lambda$ is the diagonal matrix of eigenvalues of $A$. Perform a change of variables 
  \begin{equation*}
    A \leftrightarrow (\vec{\lambda}, \dot{U}),
  \end{equation*}
  where $\vec{\lambda}$ is the vector of eigenvalues and $\dot{U}=U \mod T$, $T$ the torus of diagonal matrices, which stabilize $\vec{\lambda}$. The matrix $\dot{U}$ represents the rotation to the eigenbasis of $A$.
  
  The Jacobian is $dA=(?) d\vec{\lambda} d \dot{U}$, where $d\dot{U}$ is Haar measure. We note that the Jacobian is independent of $U$ since Lebesuge measure is rotationally invariant.
  
  So we can take $U=1+X$, $X$ ``small''. Then $U^*=U^{-1}=1-X+$ small order terms. Since $A=U\Lambda U^*$, 
  \begin{equation*}
    A+dA=(1+X)(\Lambda+d\Lambda)(1-X)=\Lambda+d\Lambda+X\Lambda-\Lambda X+ \text{ small terms}.
  \end{equation*}
  Hence
  \begin{align*}
    ...
    d a_{ii} &= d\lambda_i.
  \end{align*}
  The $x_{jk}$ are distributed according to Haar measure. Thus 
  \begin{equation*}
    dA=\prod d\lambda_{i} \prod_{j<k} d x_{jk} \prod_{j<k} |\lambda_j-\lambda_k|^2,
  \end{equation*}
  so the Jacobian is $\prod_{j<k} |\lambda_j-\lambda_k|^2$.
  
  The theorem then follows from using $f(A)=e^{-\frac{1}{2} \mathrm{tr}(A^2)}$.
\end{proof}
