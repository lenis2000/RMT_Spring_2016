...
\subsection{Examples of other random matrix ensembles with explicit eigenvalue density} % (fold)
...
\end{equation*}
\begin{exercise}
  Show that $V(\lambda)=\pm \det [\lambda_i^{j-1}]_{i,j=1}^N.$
\end{exercise}
...
if $a_{ii}\sim N(0,2)$ and $a_{ij}=a_{ji}\sim N(0,1)$ for $i<j$. Then the GOE eigenvalue density is 
\begin{equation*}
...
\end{equation*}
on the real line.
Let $y_i \sim \chi_{i\beta}$, $\xi_i\sim N(0,1)$ be independent random variables. Then the tridiagonal ensemble 
\begin{equation*}
...
\begin{remark}
  ...
  $a_{ij}\sim N(0,1)$ are i.i.d. for all $i,j$, then the spectrum
...
\subsection{Rigorous asymptotics via contour integrals} % (fold)
\label{sub:rigorous_asymptotics_via_contour_integrals}

% subsection rigorous_asymptotics_via_contour_integrals (end)

\subsection{Edge asymptotics and the Tracy--Widom distribution} % (fold)
...
should be \emph{universal} --- that is,
...
is an important ongoing program, see, e.g.,
\cite{Johansson2001Universality}, \cite{tao2011univ},
\cite{TaoVu2012Survey}, \cite{erdHos2011universality}.
Addressing some universality results is the next big goal of the course.
